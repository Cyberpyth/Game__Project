...
class AnimateSprite(pygame.sprite.Sprite):
        ...
        def animate(self, loop=False):
            ...
            #verifier si l'anmation est active
            if self.animation:
                ...
                if self.current_image >= len(self.images):
                    ...
                    #verifier si lanimation n'est pas en mode bloucle
                    if loop is False:
                        # desactivation de l'animation
                        self.animation = False

                # modifier l'image precedentee par la suivante
                self.image = self.images[self.current_image]
                self.image = pygame.transform.scale(self.image, self.size)

# definir une fonction pour charger les images d'un sprite
def load_animation_images(sprite_name):
    #charger les 24 images de ce sprit dans le dossier correspondant
    images = []
    # recuperer le chemin du dossier pour ce sprit
